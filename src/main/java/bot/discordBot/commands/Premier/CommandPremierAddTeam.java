    ...
    public void run(MessageCreateEvent event, Command command, String[] args){
        if (args.length != 3) {
            ...
            EmbedBuilder embed = new EmbedBuilder()
                    ...
                    .addField("Exemple syntaxe:","```!premier -addteam ningen @nomCapitaine```")
                    .setColor(Color.orange);
            event.getChannel().sendMessage(embed);
            return;
        }
        try{
            String idCapitaine = event.getMessage().getMentionedUsers().get(0).getIdAsString();
            ...
            EmbedBuilder embed = new EmbedBuilder()
                    .setTitle("❌   Erreur :")
                    .addField("Impossible de crée la team premier.","```Code : "+e+"```")
                    .setColor(Color.red);
            event.getChannel().sendMessage(embed);
        }
