package botdiscord.gigabot.utils.API;
...
public class ValorantApi {
    ...
    public ValorantApi() throws SQLException {
     ...
     */
    public HttpResponse<String> request(CommandContext ctx, String url){
        try{
            ...
            HttpRequest request = HttpRequest.newBuilder()
                    ...
                    .build();
            logs.writeLog(LevelLog.OK, ValorantApi.class.getName(),"Requête API Riot réussie");
            return client.send(request, HttpResponse.BodyHandlers.ofString());
        }catch (Exception e){
            logs.writeLog(LevelLog.ERR, Main.class.getName(),"Requête API Riot échoue : "+e);
        }
        return null;
    }
     ...
     */
    public String getRankTxtByInt(int nb){
        return switch (nb) {
            case 0 -> "Unranked";
            case 3 -> "Iron 1";
            case 4 -> "Iron 2";
     ...
     */
    public Color getColorRankByRankTxt(String rank){
    ...
    public boolean valorantPseudoExiste(String pseudo){
        String[] liste = pseudo.split("#");
        String url = "https://api.henrikdev.xyz/valorant/v1/mmr/eu/" + liste[0] + "/" + liste[1];
        try{
            HttpClient client = HttpClient.newBuilder()
                    .connectTimeout(Duration.ofSeconds(10))
                    .build();
